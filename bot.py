...
@bot.event
async def on_ready():
    await bot.change_presence(status=discord.Status.online, activity=discord.Game('Made by - DsquadZGaminG \nHelp - d/help')) 
    print("Bot is ready...")
...
async def link(ctx):
    masked_link_embed = discord.Embed(
        ...
        description='[CLICK HERE](YOUR LINK)'
    )
...
async def invite(ctx):
    masked_link_embed = discord.Embed(
        ...
        description='[CLICK HERE](YOUR BOT LINK)',
    )
    await ctx.send(embed=masked_link_embed)
